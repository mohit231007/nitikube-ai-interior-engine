...
st.warning(
    "Counter depth, passage width and work-triangle thresholds are visible design-scenario inputs. They are not presented as universal ergonomic/building standards unless a sourced standards layer supplies that provenance."
)
...
if geometry_mode == "Manual rectangular kitchen":
    ...
    if geometry_file:
        try:
            _, rooms, openings, _ = geometry_from_project_json(geometry_file.getvalue().decode("utf-8"))
            ...
            if labels:
                ...
                if len(selected_verified_room.polygon_ft) != 4 or len(unique_x) != 2 or len(unique_y) != 2:
                    st.error("Current kitchen generator supports axis-aligned rectangular verified rooms. NitiKube will not silently replace an arbitrary verified polygon with its bounding box.")
                else:
...
st.subheader("2 · Counter and work-centre dimensions")
...
selected_families = st.multiselect("Generate layout families", list(family_map), default=list(family_map))
include_kinds = tuple(family_map[label] for label in selected_families)
...
with st.expander("Work-triangle constraints", expanded=True):
    st.write("Enable only the limits you want treated as hard constraints. Values here are scenario inputs, not hidden NitiKube standards.")
    t1, t2 = st.columns(2)
...
if room is not None and st.button("Generate kitchen candidates", type="primary", disabled=not bool(include_kinds)):
    try:
        ...
        st.session_state["kitchen_ranked"] = rank_kitchens(room, candidates, keepouts=keepouts, requirements=requirements)
    except Exception as exc:
...
if ranked and room is not None:
    ...
    if feasible:
        ...
        selected_label = st.selectbox("Inspect feasible kitchen", list(option_map))
        ...
        k4.metric("Walkable connectivity", f"{evaluation.circulation_connectivity:.1%}" if evaluation.circulation_connectivity is not None else "Not tested")
        if evaluation.work_triangle:
            t = evaluation.work_triangle
            st.write(f"**Work triangle:** sink↔hob `{t.sink_to_hob_ft:.2f} ft`, hob↔fridge `{t.hob_to_fridge_ft:.2f} ft`, fridge↔sink `{t.fridge_to_sink_ft:.2f} ft`, total `{t.perimeter_ft:.2f} ft`.")
        ...
        st.caption("These are geometric envelopes, not a manufacturing cut list. Carcass construction, appliance voids, corner hardware, panel thickness, standard module/sheet sizes and manufacturer installation rules remain separate inputs.")

        st.download_button("Download kitchen layout SVG", svg.encode("utf-8"), f"{candidate.layout_id}_kitchen.svg", "image/svg+xml")
        ...
        payload = {"options": [{
            ...
            "features": ["geometry-checked-kitchen", f"layout-kind:{candidate.kind.value}", "work-triangle-calculated", "counter-quantity-calculated"],
            "feasible": evaluation.feasible,
            "score_source": score_source,
            "notes": [f"geometry_score={evaluation.geometry_score}", f"counter_run_ft={evaluation.gross_counter_run_ft}", "User/evidence scores remain separate from geometry score."],
        }]}
        st.download_button(
        ...
        st.error("No generated kitchen survives the current geometry/opening/passage/work-triangle constraints. Change explicit inputs or room assumptions; NitiKube will not force an invalid kitchen through the optimizer.")
else:
    st.info("Provide room geometry and generate candidates. No kitchen recommendation is precomputed or hardcoded.")

st.caption("Next fidelity steps: exact door swings/windows, plumbing/drain/gas/electrical service points, cabinet module libraries, corner logic, slab nesting, appliance manufacturer clearances, ventilation/hood requirements and sourced kitchen standards.")
